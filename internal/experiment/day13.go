...
func RunDay13() (string, error) {
	...
	for _, text := range []string{"Сделаем 20 дефлекторов.", "/task pause", "/task resume", "Нет, 25.", "Запускай.", "Поставь пока на паузу."} {
...
{{range .Rows}}<article><p><b>USER:</b> {{.Input}}</p>{{if .Response}}<pre>{{.Response}}</pre>{{end}}{{if .Error}}<p><b>Отклонено:</b> {{.Error}}</p>{{end}}{{if .State}}<div class="state">{{.State.Phase}} / {{.State.CurrentStep}} · {{.State.Status}}<br>Expected: {{.State.ExpectedAction}}<br>Товар: {{.State.State.ProductName}} · План: {{.State.State.Quantity}} · Версия: {{.State.Version}}</div>{{end}}</article>{{end}}
