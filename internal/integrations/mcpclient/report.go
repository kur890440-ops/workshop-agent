...
func Print(w io.Writer, d Discovery) error {
	...
	_, err := fmt.Fprintf(w, "WB write tools exposed: %d\nLocal mutation tools: %d\nMCP tool discovery: OK\nSession closed: %t\nServer session closed: %t\n", d.WriteToolsExposed, d.LocalMutationTools, d.SessionClosed, d.ServerClosed)
	return err
}
...
<div class="flow"><div class="box">Workshop Agent MCP Client</div>→<div class="box">in-memory</div>→<div class="box">WB MCP Server</div>→<div class="box">Existing Go WB API Client</div>→<div class="box">Wildberries API<br><small>не вызывался</small></div></div>
<section><h2>Фактический протокольный результат</h2><p>Server: {{.Server}} · Protocol: {{.Protocol}} · Transport: {{.Transport}}</p><p>Session closed: {{.SessionClosed}} · Server session closed: {{.ServerClosed}}</p><table><tr><th>Tool из tools/list</th><th>Описание</th><th>READ ONLY</th></tr>{{range .Tools}}<tr><td><code>{{.Name}}</code></td><td>{{.Description}}</td><td>{{.ReadOnly}}</td></tr>{{end}}</table></section>
<section><h2>Границы проверки</h2><p>Один процесс, MCP initialize и tools/list через официальный in-memory transport. Клиент WB создан без токена. TCP listener, реальные WB/Telegram запросы и LLM tool calling не использовались. Аннотации не заменяют проверку прав.</p><p>Полный ответ discovery со схемами: <a href="discovery.json">discovery.json</a>.</p></section></html>`))
